...
<head>
    ...
    <style>
        ...
        .table-amort tr:nth-child(even) td { background: #eff6ff; }
        ...
        .info-item { display: flex; gap: 6px; font-size: 13px; align-items: baseline; }
        .info-label { font-weight: 600; color: #374151; white-space: nowrap; flex-shrink: 0; }
    ...
    <main class="max-w-screen-xl mx-auto px-4 py-6">
        ...
        <div class="mb-5 flex items-center justify-between flex-wrap gap-3">
            ...
            <div class="flex items-center gap-2">
                <a href="/wiser-financiera-project/api/cotizaciones/exportar_pdf.php?id=<?= $id ?>"
                   target="_blank"
                   class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-700
                          bg-red-50 border border-red-200 rounded-lg hover:bg-red-100 transition-colors">
                    <i data-lucide="file-down" class="w-4 h-4"></i>
                    Exportar PDF
                </a>
                <a href="/wiser-financiera-project/api/cotizaciones/exportar_excel.php?id=<?= $id ?>"
            ...
            <div class="p-6 border-b border-gray-100">
                <div class="flex items-start justify-between mb-4">
                    ...
                    <div class="text-right">
                        <p class="text-xs text-gray-400 uppercase tracking-wide font-semibold mb-0.5">Pago Mensual</p>
                        <p class="text-3xl font-bold text-blue-900">
                            $<?= number_format((float)$cot['pago_mensual'], 2) ?>
                        </p>
                        <p class="text-sm text-gray-500">por <?= $cot['plazo_meses'] ?> meses</p>
                    </div>
                </div>
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-val font-semibold">$<?= number_format((float)$cot['total_a_pagar'], 2) ?></span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">Fecha Inicio:</span>
                        <span class="info-val"><?= CalculadoraAmortizacion::formatearFechaMx($cot['fecha_inicio']) ?></span>
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-val"><?= CalculadoraAmortizacion::formatearFechaMx($cot['fecha_limite_pago']) ?></span>
                    </div>
                    <div class="info-item">
                    ...
                    <tbody>
                        <?php foreach ($periodos as $p): ?>
                            <tr>
                                ...
                                <td class="text-center"><?= CalculadoraAmortizacion::formatearFechaMx($p['fecha_vencimiento']) ?></td>
                                <td class="text-center"><?= CalculadoraAmortizacion::formatearFechaMx($p['fecha_corte']) ?></td>
                                ...
                                <td class="num">$<?= number_format((float)$p['importe_comision'],  2) ?></td>
                                <td class="num">$<?= number_format((float)$p['excedente_pagado'],  2) ?></td>
                                <td class="num">$<?= number_format((float)$p['pago_anticipado'],   2) ?></td>
                                <td class="num font-semibold">$<?= number_format((float)$p['pago_calculado'], 2) ?></td>
                                <td class="num font-semibold text-blue-900">$<?= number_format((float)$p['pago_integrado'], 2) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                    <tfoot>
                        <tr style="background:#dbeafe; font-weight:700;">
                            <td colspan="4" class="px-3 py-2 text-center text-sm">TOTALES</td>
                            <td class="num">—</td>
                            <td class="num">$<?= number_format(array_sum(array_column($periodos, 'pago_capital')),      2) ?></td>
                            <td class="num">$<?= number_format(array_sum(array_column($periodos, 'interes_ordinario')), 2) ?></td>
                            <td class="num">$<?= number_format(array_sum(array_column($periodos, 'iva_interes')),       2) ?></td>
                            <td class="num">$<?= number_format(array_sum(array_column($periodos, 'importe_comision')),  2) ?></td>
                            <td class="num">$<?= number_format(array_sum(array_column($periodos, 'excedente_pagado')),  2) ?></td>
                            <td class="num">$<?= number_format(array_sum(array_column($periodos, 'pago_anticipado')),   2) ?></td>
                            <td class="num">$<?= number_format(array_sum(array_column($periodos, 'pago_calculado')),    2) ?></td>
                            <td class="num text-blue-900">$<?= number_format(array_sum(array_column($periodos, 'pago_integrado')), 2) ?></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
