...
<head>
    ...
    <style>
        ...
        .card-shadow {
            box-shadow: 0 8px 32px rgba(74, 144, 226, 0.12), 0 2px 8px rgba(0, 0, 0, 0.06);
        }
...
<body class="login-bg min-h-screen flex items-center justify-center px-4">
    ...
    <div class="w-full max-w-md">
        ...
        <div id="form-login" class="bg-white rounded-2xl card-shadow p-8 fade-up" style="animation-delay:0.1s">
            ...
            <?php if ($error): ?>
                <div
                    class="mb-5 flex items-start space-x-2 p-3 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm">
            ...
            <?php if ($success): ?>
                <div
                    class="mb-5 flex items-start space-x-2 p-3 bg-green-50 border border-green-200 rounded-xl text-green-700 text-sm">
                    <svg class="w-4 h-4 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
            ...
            <form id="loginForm" method="POST" novalidate>
                ...
                <div class="space-y-5">
                    ...
                    <button type="submit" id="btnLogin" class="btn-accent w-full text-white py-3 px-4 rounded-xl font-semibold text-sm
        ...
        <div id="form-forgot" class="bg-white rounded-2xl card-shadow p-8 fade-up hidden">
            ...
            <form id="forgotForm" method="POST" novalidate>
                ...
                <div class="space-y-4">
                    <div>
                        <label for="forgotEmail" class="block text-sm font-medium text-gray-700 mb-1.5">Correo
                            electrónico</label>
                        <input type="email" id="forgotEmail" name="email" required
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm
                                      focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all" placeholder="tu@correo.com">
                    </div>
                    <button type="submit" id="btnForgotSubmit" class="btn-accent w-full text-white py-3 rounded-xl font-semibold text-sm
                                   flex items-center justify-center space-x-2">
        ...
        <p class="text-center text-xs text-gray-400 mt-6">&copy; <?= date('Y') ?> <?= APP_NAME ?>. Todos los derechos
            reservados.</p>
    </div>
